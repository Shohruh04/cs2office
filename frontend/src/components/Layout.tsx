...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-cs-darker">
      {/* Header */}
      <header className="sticky top-0 z-40 bg-cs-dark/95 backdrop-blur border-b border-cs-border">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            {/* Logo */}
            <Link to="/" className="flex items-center gap-3 group">
              <CS2Logo size={44} className="transition-transform group-hover:scale-105" />
              <div>
                <h1 className="font-display font-bold text-lg text-white">
                  CS2 Stats
                </h1>
                <p className="text-xs text-gray-500">Fizmasoft Office</p>
              </div>
            </Link>
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all duration-200 ${
                      isActive
                        ? 'bg-accent-orange/10 text-accent-gold'
                        : 'text-gray-400 hover:text-white hover:bg-cs-hover'
                    }`}
                  >
                    <Icon size={18} />
